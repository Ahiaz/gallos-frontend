...
const TopBar = () => {
    ...
    const { name, balance, currency } = useSecurity();
    const firstName = name ? (name.trim().split(/\s+/)[0]).toUpperCase() : "";

    return (
        <nav className="sticky top-0 z-[1000] border-white/10 bg-[linear-gradient(90deg,#0b0b0b_0%,#181818_52%,#0b0b0b_100%)] px-0 py-3 shadow-[0_10px_35px_rgba(0,0,0,0.28)] backdrop-blur">
            <div className="w-[92%] mx-auto flex items-center justify-between">
                ...
                <div className="ml-auto flex items-center gap-2 sm:gap-4">
                    ...
                    <div className="border border-gold-400/25 bg-white/[0.06] px-3 py-2 text-right shadow-inner shadow-black/20">
                        <strong className="block text-sm leading-none text-white sm:text-base">
                            {currency}{balance}
                        </strong>
                        <div className="mt-1 hidden text-[0.62rem] font-bold uppercase tracking-[0.14em] text-white/45 sm:block">DISPONIBLE</div>
                    </div>
